...
(function() {
    ...
    function getDisplayNameFromGroup(groupRoot) {
        const time = groupRoot.querySelector("time[datetime]");
        if (!time) return null;

        const header = time.closest("div");
        if (!header) return null;

        const spans = [...header.parentElement.querySelectorAll("span span")];
    ...
    function getUserIdFromTimeEl(timeEl) {
        ...
        const groupRoot = timeEl.closest('div[class*="group"]');
    ...
    function togglePanel() {
        ...
        function renderGlobal(filter = "") {
            ...
            ALL_TIMEZONES
                ...
                .forEach(tz => {
                    ...
                    row.onmouseenter = () => {
                        if (!active) row.style.background = "rgba(255,255,255,0.07)";
                    };
                    row.onmouseleave = () => {
                        if (!active) row.style.background = "rgba(255,255,255,0.04)";
                    };
                    row.onclick = () => {
                        ZONES = active ? [] : [tz];
        ...
        addBtn.onclick = async () => {
            ...
            if (!raw) {
                statusMsg.textContent = "Enter a display name or user ID.";
                return;
            }
            if (!ALL_TIMEZONES.includes(tz)) {
                statusMsg.textContent = "Select a valid timezone first.";
                return;
            }
